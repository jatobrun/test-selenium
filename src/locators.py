...
class EducationPageLocators(object):
    FACULTY_BLOCKS = [
        (By.XPATH, f"//div[@id='views-bootstrap-accordion-{str(e)}']")
        for e in range(1, 3)
    ]
    FIRST_FACULTY_BLOCK_BUTTON = [
        (By.XPATH, f"//a[contains(@href, '#collapse-1-{e}')]") for e in range(0, 4)
    ]
    SECOND_FACULTY_BLOCK_BUTTON = [
        (By.XPATH, f"//a[contains(@href, '#collapse-2-{e}')]") for e in range(0, 4)
    ]
    FIRST_MAJOR_BLOCK = [
        (By.XPATH, f"//div[@id='collapse-1-{e}']") for e in range(0, 4)
    ]
    SECOND_MAJOR_BLOCK = [
        (By.XPATH, f"//div[@id='collapse-2-{e}']") for e in range(0, 4)
    ]
